...
def webhook(request):
    ...
    if request.method == 'POST':
        ...
        # 🛒 Add item to cart
            ...
            selected_item = parameters.get("menu_items", "").strip()
            if not selected_item:
                selected_item = user_input  # fallback
            found = False
        ...
        # ❌ Delete item from cart
            ...
            if not cart:
                ...
                response_payload = {"fulfillmentText": message}
        ...
        # 📧 Send email
            ...
            email = parameters.get('email', '').strip()
            raw_address = parameters.get('address', '')
            address = raw_address[0].strip() if isinstance(raw_address, list) else raw_address.strip()
            ...
            html_content = f"""<html>...your email HTML content...</html>"""  # You can reuse your existing HTML email here
        ...
        # 🔁 Clear cart manually
            ...
            response_payload = {
                "fulfillmentMessages": [
                    {"text": {"text": ["🧹 Your cart has been successfully cleared."]}},
                    {"text": {"text": ["🔄 No worries, let's begin a fresh order!"]}},
                    {
                        "payload": {
                            "richContent": [[
                                {
